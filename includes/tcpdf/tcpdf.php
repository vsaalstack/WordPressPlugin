...
class WPWEB_TCPDF {
	...
	public function __construct($orientation='P', $unit='mm', $format='A4', $unicode=true, $encoding='UTF-8', $diskcache=false, $pdfa=false) {
		...
		WPWEB_TCPDF_FONTS::utf8Bidi(array(), '', false, $this->isunicode, $this->CurrentFont);
		// set default font
	...
	public function GetStringWidth($s, $fontname='', $fontstyle='', $fontsize=0, $getarray=false) {
		return $this->GetArrStringWidth(WPWEB_TCPDF_FONTS::utf8Bidi(WPWEB_TCPDF_FONTS::UTF8StringToArray($s, $this->isunicode, $this->CurrentFont), $s, $this->tmprtl, $this->isunicode, $this->CurrentFont), $fontname, $fontstyle, $fontsize, $getarray);
	}
	...
	public function GetArrStringWidth($sa, $fontname='', $fontstyle='', $fontsize=0, $getarray=false) {
		...
		if ($this->isunicode AND (!$this->isUnicodeFont())) {
			$sa = WPWEB_TCPDF_FONTS::UTF8ArrToLatin1Arr($sa);
		}
		$w = 0; // total width
	...
	public function GetNumChars($s) {
		if ($this->isUnicodeFont()) {
			return count(WPWEB_TCPDF_FONTS::UTF8StringToArray($s, $this->isunicode, $this->CurrentFont));
		}
		return strlen($s);
	}
	...
	protected function getFontsList() {
		if (($fontsdir = opendir(WPWEB_TCPDF_FONTS::_getfontpath())) !== false) {
	...
	public function AddFont($family, $style='', $fontfile='', $subset='default') {
		...
		if (TCPDF_STATIC::empty_string($fontfile) OR (!@TCPDF_STATIC::file_exists($fontfile))) {
			...
			$fontfile = WPWEB_TCPDF_FONTS::getFontFullPath($tmp_fontfile, $fontdir);
			if (TCPDF_STATIC::empty_string($fontfile)) {
				$missing_style = true;
	...
	public function isCharDefined($char, $font='', $style='') {
		if (is_string($char)) {
			// get character code
			$char = WPWEB_TCPDF_FONTS::UTF8StringToArray($char, $this->isunicode, $this->CurrentFont);
			$char = $char[0];
		}
	...
	public function replaceMissingChars($text, $font='', $style='', $subs=array()) {
		...
		$fontdata = $this->AddFont($font, $style);
		$fontinfo = $this->getFontBuffer($fontdata['fontkey']);
		$uniarr = WPWEB_TCPDF_FONTS::UTF8StringToArray($text, $this->isunicode, $this->CurrentFont);
		...
		return WPWEB_TCPDF_FONTS::UniArrSubString(WPWEB_TCPDF_FONTS::UTF8ArrayToUniArray($uniarr, $this->isunicode));
	}
	...
	protected function getCellCode($w, $h=0, $txt='', $border=0, $ln=0, $align='', $fill=false, $link='', $stretch=0, $ignore_min_height=false, $calign='T', $valign='M') {
		...
		$txt = str_replace(WPWEB_TCPDF_FONTS::unichr(160, $this->isunicode), ' ', $txt);
		$prev_cell_margin = $this->cell_margin;
		$prev_cell_padding = $this->cell_padding;
		...
		if ($txt != '') {
			...
			if ($this->isunicode) {
				if (($this->CurrentFont['type'] == 'core') OR ($this->CurrentFont['type'] == 'TrueType') OR ($this->CurrentFont['type'] == 'Type1')) {
					$txt2 = WPWEB_TCPDF_FONTS::UTF8ToLatin1($txt2, $this->isunicode, $this->CurrentFont);
				} else {
					$unicode = WPWEB_TCPDF_FONTS::UTF8StringToArray($txt, $this->isunicode, $this->CurrentFont); // array of UTF-8 unicode values
					$unicode = WPWEB_TCPDF_FONTS::utf8Bidi($unicode, '', $this->tmprtl, $this->isunicode, $this->CurrentFont);
					...
					$txt2 = WPWEB_TCPDF_FONTS::arrUTF8ToUTF16BE($unicode, false);
				}
			}
			...
			if (isset($uniblock)) {
				...
				foreach ($uniblock as $uk => $uniarr) {
					if (($uk % 2) == 0) {
						...
						// character to print
						$topchr = WPWEB_TCPDF_FONTS::arrUTF8ToUTF16BE($uniarr, false);
	...
	public function getNumLines($txt, $w=0, $reseth=false, $autopadding=true, $cellpadding='', $border=0) {
		...
		$lines = 1;
		$sum = 0;
		$chars = WPWEB_TCPDF_FONTS::utf8Bidi(WPWEB_TCPDF_FONTS::UTF8StringToArray($txt, $this->isunicode, $this->CurrentFont), $txt, $this->tmprtl, $this->isunicode, $this->CurrentFont);
		...
		for ($i = 0; $i < $length; ++$i) {
			...
			if (($c != 160)
					AND (($c == 173)
						OR preg_match($this->re_spaces, WPWEB_TCPDF_FONTS::unichr($c, $this->isunicode))
						OR (($c == 45)
							AND ($i > 0) AND ($i < ($length - 1))
							AND @preg_match('/[\p{L}]/'.$this->re_space['m'], WPWEB_TCPDF_FONTS::unichr($chars[($i - 1)], $this->isunicode))
							AND @preg_match('/[\p{L}]/'.$this->re_space['m'], WPWEB_TCPDF_FONTS::unichr($chars[($i + 1)], $this->isunicode))
						)
					)
				) {
	...
	public function Write($h, $txt, $link='', $fill=false, $align='', $ln=false, $stretch=0, $firstline=false, $firstblock=false, $maxh=0, $wadj=0, $margin='') {
		...
		$chars = WPWEB_TCPDF_FONTS::UTF8StringToArray($s, $this->isunicode, $this->CurrentFont);
		// calculate maximum width for a single character on string
		$chrw = $this->GetArrStringWidth($chars, '', '', 0, true);
		array_walk($chrw, array($this, 'getRawCharWidth'));
		$maxchwidth = max($chrw);
		// get array of chars
		$uchars = WPWEB_TCPDF_FONTS::UTF8ArrayToUniArray($chars, $this->isunicode);
		// get the number of characters
		$nb = count($chars);
		// replacement for SHY character (minus symbol)
		$shy_replacement = 45;
		$shy_replacement_char = WPWEB_TCPDF_FONTS::unichr($shy_replacement, $this->isunicode);
		...
		while ($i < $nb) {
			...
			if ($c == 10) { // 10 = "\n" = new line
				...
				$tmpstr = WPWEB_TCPDF_FONTS::UniArrSubString($uchars, $j, $i);
				if ($firstline) {
					$startx = $this->x;
					$tmparr = array_slice($chars, $j, ($i - $j));
					if ($rtlmode) {
						$tmparr = WPWEB_TCPDF_FONTS::utf8Bidi($tmparr, $tmpstr, $this->tmprtl, $this->isunicode, $this->CurrentFont);
					}
				...
				if ($firstline) {
					...
					return (WPWEB_TCPDF_FONTS::UniArrSubString($uchars, $i));
				}
				++$nl;
				$j = $i + 1;
				$l = 0;
				...
				if (($c != 160)
					AND (($c == 173)
						...
						OR (($c == 45)
							AND ($i < ($nb - 1))
							AND @preg_match('/[\p{L}]/'.$this->re_space['m'], WPWEB_TCPDF_FONTS::unichr($pc, $this->isunicode))
							AND @preg_match('/[\p{L}]/'.$this->re_space['m'], WPWEB_TCPDF_FONTS::unichr($chars[($i + 1)], $this->isunicode))
						)
					)
				) {
				...
				if ($this->isUnicodeFont() AND ($arabic)) {
					...
					// *** very slow ***
					$l = $this->GetArrStringWidth(WPWEB_TCPDF_FONTS::utf8Bidi(array_slice($chars, $j, ($i - $j)), '', $this->tmprtl, $this->isunicode, $this->CurrentFont));
				} else {
				...
				if (($l > $wmax) OR (($c == 173) AND (($l + $tmp_shy_replacement_width) >= $wmax))) {
					...
					if ($sep == -1) {
						...
						if (($this->rtl AND ($this->x <= ($this->w - $this->rMargin - $this->cell_padding['R'] - $margin['R'] - $chrwidth)))
							...
							$linebreak = true;
							if ($firstline) {
								return (WPWEB_TCPDF_FONTS::UniArrSubString($uchars, $j));
							}
						} else {
						...
						$strrest = WPWEB_TCPDF_FONTS::UniArrSubString($uchars, ($sep + $endspace));
						...
						if (isset($nextstr[0]) AND ($this->GetStringWidth($nextstr[0]) > $pw)) {
							...
							$tmpstr = WPWEB_TCPDF_FONTS::UniArrSubString($uchars, $j, ($sep + $endspace));
							if ($firstline) {
								$startx = $this->x;
							...
							if ($firstline) {
								...
								$this->cell_padding = $tmpcellpadding;
								return (WPWEB_TCPDF_FONTS::UniArrSubString($uchars, ($sep + $endspace)));
							}
							$i = $sep;
							$sep = -1;
							$shy = false;
		...
		if ($l > 0) {
			...
			$tmpstr = WPWEB_TCPDF_FONTS::UniArrSubString($uchars, $j, $nb);
			if ($firstline) {
				$startx = $this->x;
			...
			if ($firstline) {
				$this->cell_padding = $tmpcellpadding;
				return (WPWEB_TCPDF_FONTS::UniArrSubString($uchars, $nb));
			}
			++$nl;
		}
		if ($firstline) {
	...
	protected function getInternalPageNumberAliases($a= '') {
		...
		if ($this->isunicode) {
			$alias['u'][] = TCPDF_STATIC::_escape(WPWEB_TCPDF_FONTS::UTF8ToLatin1($u, $this->isunicode, $this->CurrentFont));
			$alias['u'][] = TCPDF_STATIC::_escape(WPWEB_TCPDF_FONTS::utf8StrRev($u, false, $this->tmprtl, $this->isunicode, $this->CurrentFont));
			$alias['a'][] = TCPDF_STATIC::_escape(WPWEB_TCPDF_FONTS::UTF8ToLatin1($a, $this->isunicode, $this->CurrentFont));
			$alias['a'][] = TCPDF_STATIC::_escape(WPWEB_TCPDF_FONTS::utf8StrRev($a, false, $this->tmprtl, $this->isunicode, $this->CurrentFont));
		}
			...
			foreach ($alias as $a) {
				...
				if (($pos = strpos($page, $a)) !== false) {
					...
					if ($type == 'u') {
						...
						$shift = WPWEB_TCPDF_FONTS::UTF8ToUTF16BE($shift, false, $this->isunicode, $this->CurrentFont);
					} else {
	...
	protected function _putpages() {
		...
		$ptpu = WPWEB_TCPDF_FONTS::UTF8ToUTF16BE($ptpa, false, $this->isunicode, $this->CurrentFont);
		...
		for ($n = 1; $n <= $num_pages; ++$n) {
			...
			$pnpu = WPWEB_TCPDF_FONTS::UTF8ToUTF16BE($pnpa, false, $this->isunicode, $this->CurrentFont);
			...
			if (!empty($this->pagegroups)) {
				if (isset($this->newpagegroup[$n])) {
					...
					$ptgu = WPWEB_TCPDF_FONTS::UTF8ToUTF16BE($ptga, false, $this->isunicode, $this->CurrentFont);
					$ptg_num_chars = $this->GetNumChars($ptga);
				}
				++$pagegroupnum;
				$pnga = TCPDF_STATIC::formatPageNumber($pagegroupnum);
				$pngu = WPWEB_TCPDF_FONTS::UTF8ToUTF16BE($pnga, false, $this->isunicode, $this->CurrentFont);
	...
	protected function _putfonts() {
		...
		foreach ($this->FontFiles as $file => $info) {
			...
			$fontfile = WPWEB_TCPDF_FONTS::getFontFullPath($file, $info['fontdir']);
			if (!TCPDF_STATIC::empty_string($fontfile)) {
				...
				if ((!$compressed) AND (isset($info['length2']))) {
					...
					$font = WPWEB_TCPDF_FONTS::_getTrueTypeFontSubset($font, $subsetchars);
	...
	protected function _puttruetypeunicode($font) {
		...
		$out .= "\n".WPWEB_TCPDF_FONTS::_putfontwidths($font, 0);
		...
		if (isset($font['ctg']) AND (!TCPDF_STATIC::empty_string($font['ctg']))) {
			...
			// search and get ctg font file to embedd
			$fontfile = WPWEB_TCPDF_FONTS::getFontFullPath($ctgfile, $fontdir);
			if (TCPDF_STATIC::empty_string($fontfile)) {
	...
	protected function _putcidfont0($font) {
		...
		$out .= ' /FontDescriptor '.($this->n + 1).' 0 R';
		$out .= ' /DW '.$font['dw'];
		$out .= "\n".WPWEB_TCPDF_FONTS::_putfontwidths($font, $cidoffset);
		$out .= ' >>';
		$out .= "\n".'endobj';
		$this->_out($out);
		$this->_newobj();
	...
	protected function _putXMP() {
		...
		$this->encrypted = false;
		// set XMP data
		$xmp = '<?xpacket begin="'.WPWEB_TCPDF_FONTS::unichr(0xfeff, $this->isunicode).'" id="W5M0MpCehiHzreSzNTczkc9d"?>'."\n";
	...
	protected function _textstring($s, $n=0) {
		if ($this->isunicode) {
			...
			$s = WPWEB_TCPDF_FONTS::UTF8ToUTF16BE($s, true, $this->isunicode, $this->CurrentFont);
		}
	...
	protected function _fixAES256Password($password) {
		...
		$psw_array = WPWEB_TCPDF_FONTS::utf8Bidi(WPWEB_TCPDF_FONTS::UTF8StringToArray($password, $this->isunicode, $this->CurrentFont), $password, $this->rtl, $this->isunicode, $this->CurrentFont);
		foreach ($psw_array as $c) {
			$psw .= WPWEB_TCPDF_FONTS::unichr($c, $this->isunicode);
		}
	...
	public function getHTMLFontUnits($val, $refsize=12, $parent_size=12, $defaultunit='pt') {
		$refsize = WPWEB_TCPDF_FONTS::getFontRefSize($refsize);
		$parent_size = WPWEB_TCPDF_FONTS::getFontRefSize($parent_size, $refsize);
		switch ($val) {
			case 'xx-small': {
				$size = ($refsize - 4);
				break;
...
Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
	...
	protected function putHtmlListBullet($listdepth, $listtype='', $size=10) {
		...
		switch ($listtype) {
			...
			case 'lower-greek': {
				$textitem = WPWEB_TCPDF_FONTS::unichr((945 + $this->listcount[$this->listnum] - 1), $this->isunicode);
				break;
			}
			/*
		...
		if (!TCPDF_STATIC::empty_string($page)) {
			for ($p = $page_first; $p <= $page_last; ++$p) {
				...
				for ($n = 1; $n <= $maxpage; ++$n) {
					...
					$nu = WPWEB_TCPDF_FONTS::UTF8ToUTF16BE($na, false, $this->isunicode, $this->CurrentFont);
					// replace aliases with numbers
					foreach ($pnalias['u'] as $u) {
						$sfill = str_repeat($filler, max(0, (strlen($u) - strlen($nu.' '))));
						if ($this->rtl) {
							$nr = $nu.WPWEB_TCPDF_FONTS::UTF8ToUTF16BE(' '.$sfill, false, $this->isunicode, $this->CurrentFont);
						} else {
							$nr = WPWEB_TCPDF_FONTS::UTF8ToUTF16BE($sfill.' ', false, $this->isunicode, $this->CurrentFont).$nu;
						}
	...
	*/
	...
	protected function hyphenateWord($word, $patterns, $dictionary=array(), $leftmin=1, $rightmin=2, $charmin=1, $charmax=8) {
		...
		$word_string = WPWEB_TCPDF_FONTS::UTF8ArrSubString($word, '', '', $this->isunicode);
		...
		if (isset($dictionary[$word_string])) {
			return WPWEB_TCPDF_FONTS::UTF8StringToArray($dictionary[$word_string], $this->isunicode, $this->CurrentFont);
		}
		...
		for ($pos = 0; $pos < $maxpos; ++$pos) {
			...
			for ($i = 1; $i <= $imax; ++$i) {
				$subword = strtolower(WPWEB_TCPDF_FONTS::UTF8ArrSubString($tmpword, $pos, ($pos + $i), $this->isunicode));
				if (isset($patterns[$subword])) {
					$pattern = WPWEB_TCPDF_FONTS::UTF8StringToArray($patterns[$subword], $this->isunicode, $this->CurrentFont);
	...
	public function hyphenateText($text, $patterns, $dictionary=array(), $leftmin=1, $rightmin=2, $charmin=1, $charmax=8) {
		...
		$unichars = WPWEB_TCPDF_FONTS::UTF8StringToArray($text, $this->isunicode, $this->CurrentFont);
		// for each char
		...
		return WPWEB_TCPDF_FONTS::UTF8ArrSubString($txtarr, '', '', $this->isunicode);
	}
