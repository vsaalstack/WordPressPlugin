...
if( isset( $_GET['woo_vou_start_date'] ) && !empty( $_GET['woo_vou_start_date'] ) ) {
	...
	$generatecsvurl_args['woo_vou_start_date']				= $_GET['woo_vou_start_date'];
}

if( isset( $_GET['woo_vou_end_date'] ) && !empty( $_GET['woo_vou_end_date'] ) ) {
	$generatepdfurl_args['woo_vou_end_date']				= $_GET['woo_vou_end_date'];
	$generatecsvurl_args['woo_vou_end_date']				= $_GET['woo_vou_end_date'];
}
...
<a href="<?php echo esc_url($generatepdfurl); ?>" id="woo-vou-pdf-btn" class="woo-vou-btn-front woo-vou-grnpdf-btn" title="<?php esc_html_e('Generate PDF','woovoucher'); ?>"><?php echo esc_html__( 'Generate PDF', 'woovoucher' ); ?></a>
...
<table class="woo-vou-purchased-codes-table wp-list-table widefat fixed striped purchasedvous">
	<thead>
		<tr class="woo-vou-purchased-codes-table-row-head">
				<?php 
						//do action to add header title of orders list before
						do_action('woo_vou_purchased_codes_header_before');
				?>
				<th width="15%" scope="col" id='code' class="manage-column column-code column-primary sortable asc"><?php esc_html_e( 'Voucher Code','woovoucher' );?></th>
				<th width="20%" scope="col" id='product_info' class="manage-column column-product_info"><?php esc_html_e( 'Product Information','woovoucher' );?></th>
				...
				<th width="15%" scope="col" id='code_details' class="manage-column column-code_details"><?php esc_html_e( 'View Details','woovoucher' );?></th>
				<?php 
						//do action to add header title of orders list after
						do_action('woo_vou_purchased_codes_header_after');
				?>
		</tr>
	</thead>
	...
	<?php	
	if( empty( $result_arr ) ) {
		echo "<tr><td colspan='5' class='woo-vou-no-record-message'>" . esc_html__( 'No unredeemed voucher codes yet.','woovoucher' ) . "</td></tr>";
	} else {
		...
		?>
			<tr class="woo-vou-purchased-codes-row-body">
				<?php 
						//do action to add row for orders list before
						do_action( 'woo_vou_purchased_codes_row_before', $value );
				?>
				<td class="woo-vou-purchased-code-list-codes code column-code has-row-actions column-primary"  data-colname="Voucher Code"><?php 
					echo woo_vou_secure_voucher_code( $value['code'],$value['ID']);
					 do_action('woo_vou_after_purchased_voucher_code_column', $value['ID'],$value['code']);?> <button type="button" class="toggle-row"><span class="screen-reader-text"><?php esc_html_e( 'Show more details', 'woovoucher' ); ?></span></button></td>
				<?php 
				
				if( empty( $order ) ) { ?>
				<td colspan="4" class="woo-vou-purchased-code-list-codes code column-code has-row-actions column-primary order-deleted-column"  data-colname="Voucher Code"><?php esc_html_e('Order of this voucher code doesn\'t exist.','woovoucher')?></td>
				<?php 
				}
				else{
				?>
				...
				<?php } // if order found
					...
					do_action( 'woo_vou_purchased_codes_row_after', $value ); 
				?>
			</tr>
	<?php	} }  ?>
	</tbody>
	<tfoot>
		<tr class="woo-vou-purchased-codes-row-foot">
			<?php 
					//do action to add row in footer before
					do_action('woo_vou_purchased_codes_footer_before');
			?>
			<th width="15%" scope="col" id='code' class="manage-column column-code column-primary sortable asc"><?php esc_html_e( 'Voucher Code','woovoucher' );?></th>
			<th width="20%" scope="col" id='product_info' class="manage-column column-product_info"><?php esc_html_e( 'Product Information','woovoucher' );?></th>
			<th width="25%" scope="col" id='buyers_info' class="manage-column column-buyers_info"><?php esc_html_e( "Buyer's Information",'woovoucher' );?></th>
			...
			<?php 
					...
					do_action('woo_vou_purchased_codes_footer_after');
			?>
		</tr>
	</tfoot>
</table>
<!-- Code for paging starts -->
<div class="woo-vou-paging woo-vou-purchased-codes-paging">
...
<div class="woo-vou-purchased-codes-loader woo-vou-purchasedcodes-loader">
	<img src="<?php echo esc_url(WOO_VOU_IMG_URL);?>/loader.gif"/>
</div><!--.woo-vou-purchasedcodes-loader-->