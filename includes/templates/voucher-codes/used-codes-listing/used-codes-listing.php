...
<table class="woo-vou-used-codes-table wp-list-table widefat fixed striped purchasedvous">
	<thead>
		<tr class="woo-vou-used-codes-table-row-head">
				...
				?>
				<th width="13%" scope="col" id='code' class="manage-column column-code column-primary sortable asc"><?php esc_html_e( 'Voucher Code','woovoucher' );?></th>
				...
				<th width="20%" scope="col" id='buyers_info' class="manage-column column-buyers_info"><?php esc_html_e( "Buyer's Information",'woovoucher' );?></th>
				<th width="20%" scope="col" id='order_info' class="manage-column column-order_info"><?php esc_html_e( 'Order Information','woovoucher' );?></th>
				<th width="17%" scope="col" id='order_info' class="manage-column column-order_info"><?php esc_html_e( 'Redeem Information','woovoucher' );?></th>
	...
	<?php	
	if( empty( $result_arr ) ) {
		echo "<tr><td colspan='5' class='woo-vou-no-record-message'>" . esc_html__( 'No redeemed voucher codes yet.','woovoucher' ) . "</td></tr>";
	} else {
		...
		?>
			<tr class="woo-vou-used-codes-row-body">
				<?php 
						...
						do_action( 'woo_vou_used_codes_row_before', $value ); 
				?>
				<td class="woo-vou-used-code-list-codes code column-code has-row-actions column-primary"  data-colname="Voucher Code"><?php 	echo $value['code']; ?> <button type="button" class="toggle-row"><span class="screen-reader-text"><?php esc_html_e( 'Show more details', 'woovoucher' ); ?></span></button></td>
				<?php 
				
				if( empty( $order ) ) { ?>
				<td colspan="5" class="woo-vou-used-code-list-codes code column-code has-row-actions column-primary order-deleted-column"  data-colname="Voucher Code"><?php esc_html_e('Order of this voucher code doesn\'t exist.','woovoucher')?></td>
				<?php 
				}
				else{
				?>
				...
				<td class='product_info column-product_info' data-colname="Redeem Information"><?php	echo woo_vou_display_redeem_info_html( $value['ID'], $value['order_id'] ); ?></td>
				...
				<?php } // if order found
						...
						do_action( 'woo_vou_used_codes_row_after', $value ); 
				?>
			</tr>
	<?php	} }  ?>
	</tbody>
