...
if ( !class_exists('WpwebPluginUpdateChecker') ):
 ...
 * @access public
 */
class WpwebPluginUpdateChecker {
	...
	protected function installHooks(){
		...
		if ( $this->checkPeriod > 0 ){
			...
			add_action('upgrader_process_complete', array($this, 'savewpwebpluginupdates'), 11, 2);
	...
	public function requestInfo($queryArgs = array()){
		...
		// add new filter option for add email option in URL
		$queryArgs	= apply_filters( 'wpweb_modify_request_query_arguments', $queryArgs, $this->slug, $this->pluginFile );
		...
		//add cookies option for site URL to remote request
		$options = apply_filters( 'wpweb_modify_request_remote_option', $options, $this->slug, $this->pluginFile );
	...
	/**
	 * Handles to save option for plugins whose author is WPWeb
	 * Checks for author WPWeb and will execute hook only if author matches
	 */
	public function savewpwebpluginupdates($upgrader_object, $options){
		...
		if( empty( $plugin_author ) || strtolower( $plugin_author ) != 'wpweb' ) {
		...
		$wpweb_plugin_updated = get_option( 'wpweb_plugin_updated' );
		if( !empty( $wpweb_plugin_updated ) ) {
			array_push($wpweb_plugin_updated, $plugin);
		} else {
			$wpweb_plugin_updated = array( $plugin );
		}

		update_option('wpweb_plugin_updated', $wpweb_plugin_updated);
	}
	...
	public function callpluginupdateaction(){
		...
		if( !empty( $wpweb_plugin_updated ) && is_array( $wpweb_plugin_updated ) ) {

			// Loop on each plugins
			foreach( $wpweb_plugin_updated as $update_plugin ) {
			...
			delete_option( 'wpweb_plugin_updated' );
		}
	}
	...
	public function displayManualCheckResult() {
		...
		}else if( !empty( $_GET['puc_error_message'] ) && !empty(  $_GET['puc_error'] ) 
		   && isset( $_GET['puc_slug'] ) && ($_GET['puc_slug'] == $this->slug) ) {
		   
		    printf(
		     '<div class="error"><p>%s</p></div>',
		     apply_filters( 'puc_manual_check_error_msg-' . $this->slug, $_GET['puc_error_message'], 1 )
		    );
		  }
	}
 ...
 */
if ( !class_exists('WpwebPluginUpdateChecker') ) {
	class WpwebPluginUpdateChecker extends PluginUpdateChecker_2_0 { }
}
